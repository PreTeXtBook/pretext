...
Markers are an important component of an exercise if you want to add something that will be interactive in html and Runestone.  markers include \texttt{<choice>} inside a \texttt{<choices>}  for multiple choice questions, \texttt{block} inside a \texttt{blocks} for parsons (mixed up blocks) programs, \texttt{<premise>} and \texttt{<response>} inside a \texttt{<match>} for matching, an \texttt{<area>} inside \texttt{<areas>} for clickable area, \texttt{<response>} for short answer, and \texttt{<var>} inside both \texttt{<statement>} and \texttt{setup} for fill in the blank.  See \texttt{https://pretextbook.org/examples/sample-book/annotated/interactive-exercises.html} for examples and source.
