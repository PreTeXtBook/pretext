...
// START Support for floating aside auto-reveal
// To trigger this code CSS must apply "--floating-aside: true" to the aside-like element or its ancestor
document.addEventListener("DOMContentLoaded", () => {
    const asideDetails = document.querySelectorAll("details.aside-like");
    for (const details of asideDetails) {
        const summary = details.querySelector("summary");
        
        // semaphore to prevent rapid firing of mouse events
        let floatingAsideDebounce = false;
        function debounceCheck() {
            if (floatingAsideDebounce) return true;
            floatingAsideDebounce = true;
            setTimeout(() => {
                floatingAsideDebounce = false;
            }, 300);
            return false;
        }
        ...
        summary.addEventListener('mouseenter', (e) => {
            ...
            const useFloatingAsides = getComputedStyle(summary).getPropertyValue('--floating-aside').trim() === 'true';
            if (useFloatingAsides && !details.open && !details.dataset.lockedOpen) {
                summary.click();
                //details.open = true; // Open the details element
                let contents = details.querySelector('aside');
                contents.style.display = 'inherit'; // handle none from knowl code
            }
        });
        ...
        details.addEventListener('click', (e) => {
            if (!e.isTrusted) return; // Ignore synthetic events from enter/leave events
            if (details.open) {
              // visually open, may or may not be locked open
              if (!details.dataset.lockedOpen) {
                  // set to locked mode instead of doing normal click
                  details.dataset.lockedOpen = true;
                  e.preventDefault();
                  // Stop propagation to avoid triggering other click events
                  // the mouseenter event already should have opened this
                  e.stopImmediatePropagation(); 
              } else {
                  delete details.dataset.lockedOpen;
                  // no longer locked, let event propagate to close
              }
            } else {
              // not visually open
              // mark as locked open and let event propagate to open
              details.dataset.lockedOpen = true;
            }
        },
            true // Use capture phase to capture before other handlers
        );
    }
});
// END Support for floating aside auto-reveal