...
<!-- Insert default answer checker for FITB in "exercise" and PROJECT-LIKE -->
<!-- When an evaluate element has no explicit correct test but the       -->
<!-- corresponding fillin carries a static @answer, synthesize a test   -->
<!-- element so all downstream processing follows a single code path.   -->
<!-- Only @mode='string' and @mode='number' are handled here;          -->
<!-- @mode='math' uses separate logic. --> 
