...
<xsl:template match="frontmatter[titlepage[not(titlepage-items)] or colophon[not(colophon-items)]]" mode="repair">
    <xsl:copy>
        <xsl:apply-templates select="@*" mode="repair"/>
        <bibinfo>
            <!-- Keep existing bibinfo items if present -->
            <xsl:apply-templates select="bibinfo/node()|bibinfo/@*" mode="repair"/>
        ...
        <!-- Always insert a titlepage, since templates use this to insert bibliographic information -->
        <xsl:if test="child::titlepage">
            <titlepage>
        ...
        <xsl:if test="parent::book and child::colophon">
            <colophon>
                <xsl:choose>
...
<!-- But we don't want anything left over in the author's (deprecated) colophon. -->
<xsl:template match="colophon" mode="repair"/>
