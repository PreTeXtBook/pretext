            ...
            <xsl:choose>
                ...
                <xsl:when test="not($the-webwork-rep)">
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when test="document(str:replace(concat($generated-directory-source, 'webwork/', @pi:assembly-id, '.xml'), '&#x20;', '%20'), $original)/webwork-reps">
                            <xsl:message>PTX:ERROR:   could not load WeBWorK representation file "<xsl:value-of select="$webwork-rep-uri"/>" for @pi:assembly-id "<xsl:value-of select="@pi:assembly-id"/>". However, there is a file of the same name with the right structure in the parent folder. Either move the file, or re-generate the WeBWorK representations.</xsl:message>
                        </xsl:when>
                        ...
                        <xsl:when test="document(str:replace(concat($generated-directory-source, 'webwork/webwork-representations.xml'), '&#x20;', '%20'), $original)/webwork-reps">
                            <xsl:message>PTX:ERROR:   could not load WeBWorK representation file "<xsl:value-of select="$webwork-rep-uri"/>" for @pi:assembly-id "<xsl:value-of select="@pi:assembly-id"/>". However, there is a "webwork-representations.xml" file in the parent folder. This is an old format. Re-generate the WeBWorK representations.</xsl:message>
                        </xsl:when>
                        <xsl:otherwise>
