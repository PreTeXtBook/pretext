...
<xsl:param name="author.deprecations.max-age" select="'5'" />
<xsl:variable name="deprecation-max-age">
    <xsl:choose>
        <xsl:when test="$author.deprecations.max-age = 'none'">
            <xsl:text>P100Y</xsl:text>
            <xsl:message>PTX:INFO:   checking all deprecated elements.</xsl:message>
        </xsl:when>
        <xsl:when test="string(number($author.deprecations.max-age)) = 'NaN'">
            <xsl:message>PTX:WARNING:   author.deprecations.max-age should be a integer number of years or 'none'. Using default value 5.</xsl:message>
            <xsl:text>P5Y</xsl:text>
        </xsl:when>
        <xsl:otherwise>
            <xsl:text>P</xsl:text>
            <xsl:value-of select="$author.deprecations.max-age"/>
            <xsl:text>Y</xsl:text>
            <xsl:message>PTX:INFO:   checking the last <xsl:value-of select="$author.deprecations.max-age"/> year(s) of element deprecations. Rerun with '-x author.deprecations.max-age none' stringparam to see all deprecations.</xsl:message>
        </xsl:otherwise>
    </xsl:choose>
</xsl:variable>
