...
<xsl:template match="*" mode="url">
    <xsl:choose>
        <xsl:when test="$chunk-level = 0">
            ...
            <!-- since links should just jump around the page. -->
            <!-- Not including the filename makes it easy for  -->
            <!-- for a user to rename the file later.          -->
            <xsl:apply-templates select="." mode="url-anchor"/>
        </xsl:when>
        <xsl:otherwise>
            <!-- In other other cases, we want a filename plus an -->
            <!-- anchor if the link is to the interior of a page. -->
...
<xsl:template match="*" mode="url-anchor">
    <xsl:text>#</xsl:text>
