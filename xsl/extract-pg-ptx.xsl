...
<xsl:template match="webwork//image[@pg-name]">
    ...
    <xsl:text>), width=&gt;</xsl:text>
    <xsl:value-of select="substring-before($width, '%') div 100 * $design-width"/>
...
<!-- In extract-pg.xsl this template is nonempty. Here, it only serves to  -->
<!-- increase base64 string length, so we kill it.                         -->
