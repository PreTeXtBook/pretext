...
<xsl:template match="book|article|letter|memo|introduction|conclusion|paragraphs|paragraph|frontmatter|preface|abstract|acknowledgement|biography|foreword|dedication|index-part|colophon|backmatter|exercisegroup|p|assemblage|aside|biographical|historical|case" mode="serial-number" />
...
<xsl:template match="mathbook" mode="webwork-warnings">
    ...
    <xsl:if test="$coltop">
        <xsl:message>
            ...
            <xsl:text> instances encountered.</xsl:text>
        </xsl:message>
    </xsl:if>
