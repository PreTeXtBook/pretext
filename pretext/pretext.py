...
def asymptote_conversion(
    ...
    #                the conversion of individual images.
    #
...
def _runestone_services(stringparams, ext_rs_methods):
    ...
    try:
        if ext_rs_methods and "debug.rs.version" not in stringparams:
            ...
            # data about the services
            services_response = ext_rs_methods(url=services_url, format="xml")
        else:
            # Network failure is fatal.  query_runestone_services() will raise an exception
            # if it cannot get the Runestone Services file
            services_response = query_runestone_services(services_url)
    except Exception as e:
        raise Exception(e)

    # Now services_response should be meaningful since we haven't raised an exception.
...
def query_runestone_services(services_url):
    """Query the Runestone Services file from the Runestone CDN.  Returns the response object or raises an exception if the network request fails."""
